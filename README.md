...
<p align="center">
  ...
  <a href="#-liquid-glass-设计系统">设计系统</a> •
...
- **Prompt 型技能**：注入 AI 的思维框架，内置第一性原则、乔布斯、马斯克、费曼等 13 种人物/思维视角 + 项目脚手架 + 调试流程 + 查找技能 + 女娲造人，用户在 UI 中自由勾选启用
...
受 Apple WWDC25 风格启发的毛玻璃设计语言：

- **菜单栏**：不透明浅色背景 + 深色文字 + 液态玻璃下拉面板
- **消息气泡**：毛玻璃气泡 `backdrop-blur-[40px]`，统一 `rounded-2xl`
- **全局动效**：Spring 缓动 `cubic-bezier(0.16,1,0.3,1)`
- **自定义背景图**：支持上传任意图片作为界面背景，玻璃面板自动降低模糊度（60px→15-20px）提升图片可见度

### 🎨 三套完整色系

| 主题 | 名称 | 主色调 |
|------|------|--------|
| theme1 | 深邃蓝黑（默认） | `#3b82f6` 蓝 |
| theme2 | 极简黑白 | `#ff006e` 粉 |
| theme3 | 暗紫橙金 | `#e94560` 红 |
...
| 功能 | 说明 |
|------|------|
| **AI 对话** | 流式 SSE 输出，多轮对话，历史管理，编辑与分支 |
| **顶部菜单栏** | 文件/视图/工具/帮助 + 主题切换 + API 状态 + 温度滑块 |
| **图片识别** | 双引擎：模型原生视觉 + OCR 兜底 |
| **文档解析** | PDF / Word（含表格）/ Excel 上传解析 |
| **代码高亮** | react-syntax-highlighter，200+ 语言，一键复制/保存 |
| **项目资源管理器** | 左侧文件树 + 右侧文件查看器，20+ 文件类型图标 |
| **自定义背景** | 支持上传任意图片作为界面背景，玻璃面板自动适配 |
| **更新日志** | 内置 Changelog 弹窗，版本更新一目了然 |
| **温度滑块** | 菜单栏常驻，实时调节 AI 创造性（0~1） |
...
输出：`frontend/dist-electron/煎蛋Agent Setup 2.0.0.exe`
