...
export class ErrorBoundary extends Component<Props, State> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="flex flex-col items-center justify-center h-screen app-bg select-none">
          ...
          <button
            ...
            className="px-4 py-2 text-sm bg-accent text-white rounded-2xl hover:brightness-110 transition-all"
          >
            刷新页面
          </button>
          {this.state.error && (
            <details className="mt-4 max-w-md text-xs text-gray-400 text-center">
              ...
              <pre className="mt-2 p-2 glass-sm rounded-2xl overflow-auto">{this.state.error.message}</pre>
            </details>
          )}
        </div>
      )
